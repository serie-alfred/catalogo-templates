...
**Logo never fixed.** Header/Footer sources from `.starter` often hardcode the source brand's logo (inline SVG mark, `<img src="...">` to a brand asset, or a hardcoded wordmark string). The command always strips that and replaces it with the catalog's standard dynamic-logo pattern — `const { logo } = useLayout()` (from `@/context/LayoutContext`), rendering `<img src={logo} alt="Logo" />` when set, else `<span className={styles.logoFallback}>SERIE//A</span>`. This is the exact scheme already used by `Header01`/`Header03`/`Header04`; `Header06` (migrated before this rule existed) was retrofitted to match it. Style `.logoFallback` to read theme color/font vars (`var(--header-text, var(--text-color-base, ...))`, `var(--font-primary)`) so it stays on-brand per the user's chosen palette instead of the source brand's.
