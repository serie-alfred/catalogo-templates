...
/**
 * Mede, no DOM, o elemento-FOLHA cujo texto é exatamente `conteudo`.
 *
 * Os 84 nós de texto das fixtures têm o próprio conteúdo como nome, então essa
 * é a única classe de nó que casa sozinha, sem mapa escrito à mão. Ambiguidade
 * vira status próprio em vez de medição errada — é a diferença entre não medir
 * e medir a caixa errada.
 */
export const porTexto = (page, escopoSel, conteudo) =>
  page.evaluate(
    (sel, txt) => {
      const escopo = sel ? document.querySelector(sel) : document.body;
      if (!escopo) return { erro: 'escopo não existe' };
      const alvos = [...escopo.querySelectorAll('*')].filter(
        el => !el.firstElementChild && el.textContent.trim() === txt
      );
      if (alvos.length === 0) return { erro: 'nenhum' };
      if (alvos.length > 1) return { erro: `${alvos.length} candidatos` };
...
/**
 * Nós que NÃO têm gêmeo no DOM e por isso não entram na conta de cobertura.
 *
 * Duas famílias, ambas estruturais do Figma e não do produto:
 *  - internos de ícone (`Vector`, `Group`, `Ellipse` dentro de um `lucide/*`):
 *    o React renderiza um `<svg>` só, os paths não são elementos endereçáveis;
 *  - frames de conteúdo que no CSS viraram `padding` do pai (o `Frame 31` do
 *    t5 é exatamente o `padding: 20px 24px` do `.block`).
 */
export const INTERNOS_DE_ICONE = /^(Vector|Group \d+|Ellipse \d+|Rectangle \d+|Union|Subtract)$/;
